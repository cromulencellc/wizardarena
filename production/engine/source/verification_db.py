...
class WADBEngine():
    ...
    def GetLastRoundID(self, cur_round_id):
        ...
        try:
            con_cursor.execute("SELECT MAX(id) FROM rounds WHERE finished_at != NULL AND id < %s" % (cur_round_id,))
        except psycopg2.Error as e:
    ...
    def GetPollPerformanceData( self, round_id, cs_id, team_id ):
        ...
        try:
            con_cursor.execute("SELECT pollers.id, pollers.mean_wall_time, pollers.stddev_wall_time, poll_feedbacks.wall_time, pollers.mean_max_rss, pollers.stddev_max_rss, poll_feedbacks.max_rss, pollers.mean_min_flt, pollers.stddev_min_flt, poll_feedbacks.min_flt, pollers.mean_cpu_clock, pollers.stddev_cpu_clock, poll_feedbacks.cpu_clock FROM pollers, poll_feedbacks WHERE pollers.id = poll_feedbacks.poller_id AND pollers.challenge_set_id = %s AND pollers.round_id = %s AND poll_feedbacks.team_id = %s AND poll_feedbacks.status = 'succcess'", (cs_id, round_id, team_id) )
        except psycopg2.Error as e:
    ...
    def GetSuccessfulPOVTeamCountAgainstOthersButTeamID( self, round_id, cs_id, ignore_team_id ):
        if self.db_con is None:
            raise WADBNotConnectedException("DB not connected")
        ...
        try:
            con_cursor.execute("SELECT COUNT(DISTINCT team_id) FROM proofs WHERE challenge_set_id = %s AND target_id != %s AND id IN (SELECT proof_id FROM proof_feedbacks WHERE round_id = %s AND successful = True)", (cs_id, ignore_team_id, round_id) )
        except psycopg2.Error as e:
    ...
    def AddProofFeedback( self, round_id, cs_id, team_id, from_tid, cb_seed, pov_seed, pov_success, pov_error, pov_signal, pov_type ):
        if self.db_con is None:
            raise WADBNotConnectedException("DB not connected")
